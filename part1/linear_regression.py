...
def compute_Phi(x,p):
    ...
    for a in range(0,np.size(x)):
        for b in range(0,p):
            Phi.append(x[a]**b)
    Phi=np.reshape(Phi,(np.size(x),p))
    Phi=np.asmatrix(Phi)
    #########################################
    return Phi
...
def compute_L(yhat,y):
    ...
    L = (1/(2*len(y)))*(np.matmul(np.transpose(temp),temp))
    #########################################
    return L 
...
def compute_dL_dw(y, yhat, Phi):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dL_dw = (1/len(y))*(np.matmul((np.transpose(Phi)),(np.subtract(yhat,y))))
    #########################################
    return dL_dw
...
def train(X, Y, alpha=0.001, n_epoch=100):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
        H_theta = np.matmul(X,w)

    # Back propagation: compute local gradients 
        dL_dw = compute_dL_dw(Y, H_theta, X)
